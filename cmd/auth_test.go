...
func TestAuthListReturnsAllProvidersWhenNoneConfigured(t *testing.T) {
	...
	got := stdout.String()
	want := "[{\"provider\":\"gemini\",\"configured\":false,\"default\":false},{\"provider\":\"openai\",\"configured\":false,\"default\":false}]\n"
	if got != want {
		t.Fatalf("stdout = %q, want %q", got, want)
	}
}
...
func TestAuthListShowsConfiguredProviderWithKeyPreview(t *testing.T) {
	...
	got := stdout.String()
	want := "[{\"provider\":\"gemini\",\"configured\":true,\"default\":false,\"apiKeyPreview\":\"AIza******abcd\"},{\"provider\":\"openai\",\"configured\":false,\"default\":false}]\n"
	if got != want {
		t.Fatalf("stdout = %q, want %q", got, want)
	}
}
